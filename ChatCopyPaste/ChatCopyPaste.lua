...
function CCP.openCcpCopyFrame(window, url)
	...
	--Seems this needed to be set back and forth after the midnight update for it to actally work?
	ccpCopyFrame.EditBox:SetFont("Fonts\\ARIALN.ttf", 10, "");
	...
	if (url) then
		...
		for i = startLine, maxLines do
			...
			if (issecretvalue and issecretvalue(currentMsg)) then
				isSecret = true;
				hasSecretMsg = true;
			end
			if (not isSecret) then
				...
				if (strmatch(currentMsg, "k:(%d+):(%d+):BN_WHISPER:")) then
					...
					local presenceID = strmatch(currentMsg, "k:(%d+):%d+:BN_WHISPER:");
	...
	if (hasSecretMsg) then
		print("|cFFFFFFFFChat Copy paste:|r Some lines contained a Blizzard secret value and could not be copied.");
	end
end
...
function CCP.removeChatJunk(currentMsg)
	...
	local chatNumber = strmatch(currentMsg, "(%d+) |4year:years;");
	if (chatNumber) then
		if (tonumber(number) == 0) then
			currentMsg = gsub(currentMsg, "(%d+) |4year:years;", chatNumber .. " year");
		elseif (tonumber(chatNumber) > 1) then
			currentMsg = gsub(currentMsg, "(%d+) |4year:years;", chatNumber .. " years");
		else
			currentMsg = currentMsg --Do nothing.
		end
	end
	local chatNumber = strmatch(currentMsg, "(%d+) |4day:days;");
	if (chatNumber) then
		if (tonumber(number) == 0) then
			currentMsg = gsub(currentMsg, "(%d+) |4day:days;", chatNumber .. " day");
		elseif (tonumber(chatNumber) > 1) then
			currentMsg = gsub(currentMsg, "(%d+) |4day:days;", chatNumber .. " days");
		else
			currentMsg = currentMsg --Do nothing.
		end
	end
	local chatNumber = strmatch(currentMsg, "(%d+) |4hour:hours;");
	if (chatNumber) then
		if (tonumber(number) == 0) then
			currentMsg = gsub(currentMsg, "(%d+) |4hour:hours;", chatNumber .. " hour");
		elseif (tonumber(chatNumber) > 1) then
			currentMsg = gsub(currentMsg, "(%d+) |4hour:hours;", chatNumber .. " hours");
		else
			currentMsg = currentMsg --Do nothing.
		end
	end
	local chatNumber = strmatch(currentMsg, "(%d+) |4minute:minutes;");
	if (chatNumber) then
		if (tonumber(number) == 0) then
			currentMsg = gsub(currentMsg, "(%d+) |4minute:minutes;", chatNumber .. " minute");
		elseif (tonumber(chatNumber) > 1) then
			currentMsg = gsub(currentMsg, "(%d+) |4minute:minutes;", chatNumber .. " minutes");
		else
			currentMsg = currentMsg --Do nothing.
		end
	end
	local chatNumber = strmatch(currentMsg, "(%d+) |4second:seconds;");
	if (chatNumber) then
		if (tonumber(number) == 0) then
			currentMsg = gsub(currentMsg, "(%d+) |4second:seconds;", chatNumber .. " second");
		elseif (tonumber(chatNumber) > 1) then
			currentMsg = gsub(currentMsg, "(%d+) |4second:seconds;", chatNumber .. " seconds");
		else
			currentMsg = currentMsg --Do nothing.
		end
	end
	currentMsg = gsub(currentMsg, "|T.-|t", "");
	return currentMsg;
end

function CCP.makeChatWindowButtons(i)
	if (not _G['ChatFrame'..i] or _G['ChatFrame'..i].ccpButton) then
		return;
	end
...
f:SetScript('OnEvent', function(self, event, ...)
	local config = GetCVar("whisperMode");
	if (config == "popout" or config == "popout_and_inline") then
		CCP:loadNewWhisperButtons();
	end
end)
...
local urlPattern = "[%w_.~!*:@&+$/?%%#-]-%w[-.%w]*%w%.%a%a+:?%d*/?";
local function parseMsg(msg)
	local ignored = {};
	for word in string.gmatch(msg, "%S+") do
		local match = string.match(word, urlPattern);
		if (match and not hasEscapes(word) and not ignored[word]) then
			local escaped = string.gsub(word, "([%(%)%%%+%-%*%?%[%^%$])", "%%%1")
			msg = string.gsub(msg, escaped, urlColorCode .. "(|HccpCustomLink:url|h" .. word .. "|h)|r");
			ignored[word] = true;
		end
	end
	return msg;
...
function CCP.ccpChatWebLinks(self, event, msg, author, ...)
	...
	if (strmatch(msg, "%[MDT_v2:") or strmatch(msg, "%[WeakAuras:") or not strmatch(msg, urlPattern)) then
		return;
	end
	msg = parseMsg(msg);
	--[[local ignored = {};
	for word in gmatch(msg, "%S+") do
		local match = strmatch(word, urlPattern);
		if (match and not hasEscapes(word) and not ignored[word]) then
			msg = gsub(msg, match, urlColorCode .. "(|HccpCustomLink:url|h" .. word .. "|h)|r");
			--word = gsub(word, "%?", "%%?")
			word = gsub(word, "[%(%)%.%%%+%-%*%?%[%^%$]", "%%$1")
			--msg = gsub(msg, word, urlColorCode .. "(|HccpCustomLink:url|h" .. word .. "|h)|r");
			ignored[word] = true;
		end
	end]]
	...
	--Old way.
	--[[for word in gmatch(msg, urlPattern) do
		if (not strfind(word, "%{rt%d%}")) then --Ignore urls that have a raid marker in them it causes errors, the pattern matching string needs updating one day I guess.
			msg = gsub(msg, word, urlColorCode .. "(|HccpCustomLink:url|h" .. word .. "|h)|r", 1);
		end
	end]]
